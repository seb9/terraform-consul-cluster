...
# Return the IP of each running consul server in the same subnet as mine.
function cluster-instance-ips {
    aws --region="$REGION" ec2 describe-instances --filters "Name=network-interface.subnet-id,Values=$SUBNET_ID" "Name=tag:Consul-Role,Values=server" "Name=instance-state-code,Values=16" --query="Reservations[].Instances[].[PrivateIpAddress]" --output="text"
}
...
# Wait until we have as many cluster instances as we are expecting in the other AZ (this is managed by AWS autoscaling).
REMOTE_CLUSTER_INSTANCE_IPS=$(cluster-instance-ips-other-az)

while COUNT=$(echo "$REMOTE_CLUSTER_INSTANCE_IPS" | wc -l) && [ "$COUNT" -lt "$CONSUL_SERVER_COUNT_EXPECTED" ]
do
    echo "$COUNT instances in the cluster in the other AZ, waiting for $CONSUL_SERVER_COUNT_EXPECTED instances to warm up..."
    sleep 1
    REMOTE_CLUSTER_INSTANCE_IPS=$(cluster-instance-ips-other-az)
done
...
# build the retry-join array for WAN
RETRY_JOINS_WAN=""
for i in $REMOTE_CLUSTER_INSTANCE_IPS
do 
    if ! [ "$i" == "$$IP" ]
    then
        RETRY_JOINS_WAN="$RETRY_JOINS_WAN,\"$i\""
    fi
done
RETRY_JOINS_WAN=$(echo "$RETRY_JOINS_WAN" | cut -c 2-)
